...
def download_nltk_packages():
    # Checks for and downloads all required NLTK packages
    required_packages = {
    ...
    for package_id, path in required_packages.items():
        try:
            nltk.data.find(path)
            print(f"[NLTK] '{package_id}' is already downloaded.")
        except LookupError:
            print(f"[NLTK] Downloading '{package_id}'...")
            nltk.download(package_id)
download_nltk_packages()


class Chatbot:
    def __init__(self, intents_file, confidence_threshold=0.15):
        ...
        'make', 'cook', 'with', 'for', 'and', 'a', 'an', 'some', 'any', 'something', 'thing', 'food'}
        self.stopwords = nltk_stopwords.union(custom_stopwords)
    ...
    def preprocess_text(self, text):
        ...
        return [self.lemmatizer.lemmatize(token) for token in word_tokenize(text)]
    ...
    def classify_intent(self, user_input):
        """Uses calculated probabilities to predict intent"""
    ...
    def extract_entities(self, user_input, intent):
        ...
        # Tokenizes the input text and tags each word with part-of-speech
        tagged_tokens = nltk.pos_tag(word_tokenize(user_input.lower()))
            ...
            if self.context.get('awaiting_entity'):
                ...
                if new_intent in ['greet', 'goodbye', 'help', 'affirm', 'deny', 'fallback']:
                    ...
                    if recipe_response:
                        ...
                        # If it still fails, give a generic response and clear context
